...
async function main() {
  ...
  // Fallback to local feeds if remote fetch fails
  if (!feedX) {
    const localPath = join(localFeedDir, 'feed-x.json');
    if (existsSync(localPath)) {
      feedX = JSON.parse(await readFile(localPath, 'utf-8'));
      errors.push('Using local tweet feed (remote unavailable)');
    } else {
  ...
  if (!feedPodcasts) {
    const localPath = join(localFeedDir, 'feed-podcasts.json');
    if (existsSync(localPath)) {
      feedPodcasts = JSON.parse(await readFile(localPath, 'utf-8'));
      errors.push('Using local podcast feed (remote unavailable)');
    } else {
  ...
  if (!feedBlogs) {
    const localPath = join(localFeedDir, 'feed-blogs.json');
    if (existsSync(localPath)) {
      feedBlogs = JSON.parse(await readFile(localPath, 'utf-8'));
      errors.push('Using local blog feed (remote unavailable)');
    } else {
